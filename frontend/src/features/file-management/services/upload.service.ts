...
export class UploadService {
   ...
   * 获取文件URL
   */
  getFileUrl(filename: string, type: 'avatar' | 'document'): string {
    const baseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000/api/v1';
    return `${baseUrl}/uploads/${type === 'avatar' ? 'avatars' : 'documents'}/${filename}`;
  }

  /**
   * 验证文件类型
   */
  validateFileType(file: File, type: 'avatar' | 'document'): boolean {
    if (type === 'avatar') {
   ...
   * 验证文件大小
   */
  validateFileSize(file: File, type: 'avatar' | 'document'): boolean {
    const maxSize = type === 'avatar' ? 2 * 1024 * 1024 : 5 * 1024 * 1024; // 头像2MB，文档5MB
